...
class OrderController extends Controller
{
    ...
    public function place_order(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'payment_method' => 'required|in:cash_on_delivery,digital_payment,wallet,wallet,stripe,otp,kh,mbh,paypal',
        ...
        try {
            ...
            //PlaceOrderMail
            try {
                if ($order->order_status == 'confirmed' && config('mail.status')) {
      ...
      public function stripecheck(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'order_amount' => 'required',
            'payment_method' => 'required|in:cash_on_delivery,digital_payment,wallet,stripe,otp,kh,mbh',
            'order_type' => 'required|in:take_away,delivery,parcel',
            'store_id' => 'required_unless:order_type,parcel',
            'distance' => 'required_unless:order_type,take_away',
            'address' => 'required_unless:order_type,take_away',
        ...
        $order->user_id = $request->user()->id;
        $order->order_amount = $request['order_amount'];
        $order->payment_status = ($request['payment_method'] == 'wallet' || $request['payment_method'] == 'stripe') ?  'paid' : 'unpaid';
        $order->order_status =    ($request->payment_method == 'wallet' || $request->payment_method == 'stripe' || $request->payment_method == 'cash_on_delivery') ? 'confirmed' : 'pending';
        $order->coupon_code = $request['coupon_code'];
        ...
        return response()->json([
            'errors' => [
                ['code' => 'order-place', 'message' => translate('messages.proceed_with_order')]
            ]
        ], 200);
    }
    ...
    public function cancel_order(Request $request)
    {
        ...
        if (!$order) {
            ...
            $order->save();
            if($request->reason != "Digital payment issue"){
                            Helpers::send_order_notification($order);
