...
class OrderController extends Controller
{
    public function list($status)
    {
        ...
        $orders = Order::with(['customer'])
            ...
            if ($status == 'on_going') {
                $incoming_orders = Order::with(['customer'])
                ...
                $outgoing_orders = Order::with(['customer'])
                ...
                $handover_orders = Order::with(['customer'])
                    ->where('order_status', 'handover')
                    ->StoreOrder()
                    ->where('store_id', \App\CentralLogics\Helpers::get_store_id())
                    ->orderBy('schedule_at', 'desc')
                    ->paginate(config('default_pagination'));
                return view('vendor-views.order.ongoing_list', compact('incoming_orders', 'outgoing_orders', 'handover_orders', 'orders', 'status'));
            }
